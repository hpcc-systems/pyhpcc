...
def create_compile_bash_command(repository, outputfile, filename):
    ...
    """
    try:
        return ''' eclcc -legacy  -I {0}  -platform=thor  -E -o {1} {2} -wu'''.format(repository, outputfile, filename)
    except HPCCException as e:
        raise e
...
def create_run_bash_command(compiledfile,cluster,ip,port,username,password,jobname):
    """
    ...
    """
    try:
        return '''ecl run {0} --limit=100 --wait=0 --target={1}  --server={2} --ssl --port={3} -u={4} -pw={5} --name={6} -v'''.format(compiledfile,cluster,ip,port,username,password,jobname)
    except HPCCException as e:
        raise e
