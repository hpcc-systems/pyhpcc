...
from pyhpcc.errors import HPCCAuthenticationError, HPCCException
...
class TestHPCCAPI(unittest.TestCase):
    ...
    SPRAY_VARIABLE_DEST_FILE = '' #TODO: Update with variable spray destination file name
    ...
    def test_fileList(self):
        payload = {'Netaddr':'',    #TODO: Update with landing zone IP
                   'Path': "/data", 
                   'OS':2} 
    ...
    def test_dfuQuery(self):
        ...
        if 'DFULogicalFile' in list(response['DFUQueryResponse']['DFULogicalFiles'].keys()):
            dfu_files_list = response['DFUQueryResponse']['DFULogicalFiles']['DFULogicalFile']
    ...
    def test_getdfuInfo(self):
        ...
        if 'Exceptions' in response:
            raise HPCCException(message=response['Exceptions']['Exception'][0]['Message'])
        self.assertEqual(self.LOGICAL_FILE_NAME, response['DFUInfoResponse']['FileDetail']['Name'])
    ...
    def test_get_wuinfo(self):        
        ...
        if 'Exceptions' in response:
            raise HPCCException(message=response['Exceptions']['Exception'][0]['Message'])
        self.assertEqual(self.STATIC_WUID, response['WUInfoResponse']['Workunit']['Wuid'])
    ...
    def test_getDfuWorkunitInfo(self):
        ...
        if 'Exceptions' in response:
            raise HPCCException(message=response['Exceptions']['Exception'][0]['Message'])
        self.assertEqual(self.STATIC_DFU_WUID, response['GetDFUWorkunitResponse']['result']['ID'])
    ...
    def test_getSubFileInfo(self):
        ...
        self.assertIn('subfiles', list(response['DFUInfoResponse']['FileDetail'].keys()))
        # superfiles can have no subfiles attached at times
        self.assertGreaterEqual(len(response['DFUInfoResponse']['FileDetail']['subfiles']['Item']), 0) 
    ...
    def test_wuCreateAndUpdate(self):
        ...
        if 'Workunit' in list(response['WUUpdateResponse']['Workunit'].keys()):
            self.assertEqual(job_name, response['WUUpdateResponse']['Workunit']['Jobname'])
    ...
    def test_UploadFile(self):
        ...
        file_name = os.path.join(current_directory, 'tests', 'test_files', 'pyhpcc_spray_fixed.csv')
        ...
        files = {'file': (upload_file_name, open(file_name, 'rb'), 'text/plain', {'Expires': '0'})}
        ...
        self.assertEqual(response['UploadFilesResponse']['UploadFileResults']['DFUActionResult'][0]['Result'], 'Success')
    ...
    def test_getgraph(self):
        ...
        # A DFU workunit is created when spraying the file. Response returns this wuid.
        self.assertEqual(response['WUGetGraphResponse']['Graphs']['ECLGraphEx'][0]['Label'], 'RulesFile')


    def test_get_wuquery(self):
        payload = { 
            'Wuid': self.STATIC_WUID5 
          }
        ...
        # A DFU workunit is created when spraying the file. Response returns this wuid.
        self.assertEqual(response['WUQueryResponse']['Workunits']['ECLWorkunit'][0]['Owner'], self.HPCC_USERNAME)
    ...
    def test_fileQuery(self):
        ...
        # A DFU workunit is created when spraying the file. Response returns this wuid.
        self.assertEqual(response['DFUQueryResponse']['DFULogicalFiles']['DFULogicalFile'][0]['Name'], self.LOGICAL_FILE_NAME)
